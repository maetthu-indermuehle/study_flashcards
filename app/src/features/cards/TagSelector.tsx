    ...
    <div>
      ...
      <div className="mb-3 flex flex-wrap gap-1.5">
        {selected.map((tag) => (
          <span
            key={tag.id}
            className="inline-flex items-center gap-1 rounded-full bg-sky-100 px-2.5 py-1 text-xs font-medium text-sky-800"
          >
            <span className="text-sky-500">{tag.type.toLowerCase()}</span>
            <span>{tag.name}</span>
            <button
              type="button"
        ...
        {newTags.map((tag, i) => (
          ...
          >
            <span className="text-violet-500">{tag.type.toLowerCase()}</span>
            <span>{tag.name}</span>
      ...
      {unselected.length > 0 && (
        <select
          ...
          className="mb-2 rounded-md border border-slate-300 bg-white px-3 py-1.5 text-sm text-slate-700 focus:border-sky-400 focus:outline-none"
        >
      ...
      {!showNew ? (
        ...
        <div className="mt-1 flex flex-wrap items-center gap-2">
          <input
            ...
            className="rounded-md border border-slate-300 px-3 py-1.5 text-sm focus:border-sky-400 focus:outline-none"
          />
          <select
            value={newType}
            onChange={(e) => setNewType(e.target.value as TagType)}
            className="rounded-md border border-slate-300 bg-white px-2 py-1.5 text-sm focus:border-sky-400 focus:outline-none"
          >
            {TAG_TYPES.map((t) => (
